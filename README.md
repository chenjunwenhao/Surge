...
<p align="center">
  <a href="https://github.com/chenjunwenhao/Surge/releases"><img src="https://img.shields.io/badge/Download-v2.4.0-0ea5e9?style=for-the-badge" /></a>
...
|---|---|---|---|---|---|
| 💰 价格 | **免费开源 (MIT)** | $1,099+/年 | 免费 / Pro $199 | $89 买断 | 免费 |
| ⚡ 启动 | **秒开** | 较慢 | Java 重载 | 原生快 | Java 慢 |
| 🎯 数据库 | **MySQL 唯一** | 全支持 | 全支持 | 全支持 | MySQL 唯一 |
| 📦 安装包 | 122 MB | 200+ MB | 500+ MB | 30 MB | 400+ MB |
| ✨ SQL 格式化 | **自研 400行引擎** | 内置 | 内置 | 内置 | 内置 |
| 🔒 事务保护 | **5/15/30分钟超时** | ❌ | ❌ | ❌ | ❌ |
| 📊 大表导出 | **行数预估 + 弹窗确认** | ❌ | ❌ | ❌ | ❌ |
| 💾 Edit 模式 | **零 SQL 原地编辑** | ✅ | ✅ | ✅ | ✅ |
...
<p>
多数据库工具为了兼容所有引擎，每一步操作都要兜底最差情况——SQL 补全不敢做上下文推断、格式化只能做关键字换行、导出不同你表里有多少数据、事务超时可以无限挂起。
</p>
<p>
<strong>Surge 的专一带来的是每个细节的深度打磨</strong>：<code>SELECT</code> 后的列对齐知道该补哪些表字段、JOIN 层级有语义缩进、格式化不是机器式换行是给人看的排版、导出前先查 <code>information_schema</code> 告诉你这表有多大。
</p>
